...
public class StaffMonthlyDetailRepo {
    ...
    public void clearCacheData(long currentTimeMillis) {
        Iterator<Map.Entry<Long, List<NetStaffAttendRecord>>> iterator = cacheAttendRecords.entrySet().iterator();
        Calendar targetCalendar = Calendar.getInstance();
        targetCalendar.setTimeInMillis(currentTimeMillis);
        while (iterator.hasNext()) {
            Map.Entry<Long, List<NetStaffAttendRecord>> next = iterator.next();
            Long key = next.getKey();
            Calendar entryCalendar = Calendar.getInstance();
            entryCalendar.setTimeInMillis(key);
            if (entryCalendar.get(Calendar.YEAR) == targetCalendar.get(Calendar.YEAR)
                    && entryCalendar.get(Calendar.MONTH) == targetCalendar.get(Calendar.MONTH)) {
                iterator.remove();
            }
        }
    }
